...
- **Logs**: Total log rows capped (e.g. 5,000); oldest purged when over.
...
- **SQLite**: Chosen for simplicity and zero cost; single-file, no separate DB server. For scale or multi-instance, replace with a shared store (e.g. Cosmos DB, PostgreSQL).
- **Mock AI in production**: Enables zero-cost deployment; summaries/steps are deterministic. For richer AI in private deployments, add an optional paid provider behind the same `IAIService` abstraction.
...
- Real log ingestion (e.g. file tail, webhook) alongside or instead of simulation.
